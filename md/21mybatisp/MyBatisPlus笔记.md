...
spring:
  datasource:
    ...
    url: jdbc:mysql://192.168.8.8:3306/mybatis?useSSL=false&useUnicode=true&characterEncoding=UTF-8
    username: root
    password: root1234
```
...
class Mybatisplus01QuickstartApplicationTests {
    ...
    void testSave() {
        User user = new User();
        user.setName("chinasoft程序员");
        user.setPassword("chinasoft");
        user.setAge(12);
...
思考一下Mybatis分页插件是如何用的？ PageHelper 插件 ？
...
- 相关属性
    ...
    ![1693381552100](MyBatisPlus笔记.assets/1693381552100.png)

    ![1693381583526](MyBatisPlus笔记.assets/1693381583526.png) 
...
globalConfig.setAuthor("chinasoft程序员");    //设置作者
...
老版本的使用
 ...
 ```txt
   //1. 创建代码生成器对象，执行生成代码操作
        ...
        DataSourceConfig dataSource = new DataSourceConfig();
        dataSource.setDriverName("com.mysql.cj.jdbc.Driver");
        dataSource.setUrl("jdbc:mysql://192.168.8.81:3306/news?serverTimezone=UTC");
        dataSource.setUsername("root");
        ...
        GlobalConfig globalConfig = new GlobalConfig();
//        globalConfig.setOutputDir(System.getProperty("user.dir")+"/mybatisplus_04_generator/src/main/java");    //设置代码生成位置
        globalConfig.setOutputDir("./plus-generator/src/main/java");
        globalConfig.setOpen(false);    //设置生成完毕后是否打开生成代码所在的目录
        globalConfig.setAuthor("unknow程序员");    //设置作者
        globalConfig.setFileOverride(true);     //设置是否覆盖原始生成的文件
        ...
        //设置包名相关配置
        PackageConfig packageInfo = new PackageConfig();
        packageInfo.setParent("com.chinasoft.back");   //设置生成的包名，与代码所在位置不冲突，二者叠加组成完整路径
        ...
        //策略设置
        StrategyConfig strategyConfig = new StrategyConfig();
        strategyConfig.setInclude("ev_users");  //设置当前参与生成的表名，参数为可变参数
        strategyConfig.setInclude("ev_articles");  //设置当前参与生成的表名，参数为可变参数
        strategyConfig.setInclude("ev_article_cate");  //设置当前参与生成的表名，参数为可变参数
        strategyConfig.setTablePrefix("ev_");  //设置数据库表的前缀名称，模块名 = 数据库表名 - 前缀名  例如： User = tbl_user - tbl_
